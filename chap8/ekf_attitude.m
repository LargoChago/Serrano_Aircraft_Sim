   ...
   %--------------------------------
    ...
    methods
        ...
        function self = ekf_attitude(SENSOR, SIM)
            ...
            self.Q = [.000001, 0; 0, .000001] ;
            self.Q_gyro = normrnd(0,SENSOR.gps_course_sigma^2);
            self.R_accel = normrnd(0,SENSOR.accel_sigma^2);
            self.N = 10;  % number of prediction step per sample
            self.xhat = [0,0] %[MAV.phi0, MAV.theta0] ; % initial state: phi, theta
            self.P = 0;
        ...
        function h_ = h(self, x, state)
            ...
            h_ = [state.q*state.Va*sin(theta)+9.81*sin(theta);
                -state.p*state.Va*sin(theta) - 9.81*cos(theta)*sin(theta) + state.r*state.Va*cos(theta);
        ...
        function self = propagate_model(self, state)
            ...
            for i=1:self.N
                ...
                self.P = A_d*self.P*A_d' + self.Ts^2*self.Q; 
            end
        end
